<div>
    ...
    <form wire:submit.prevent="submit" class="prose">
        ...
        <div class="mb-4">
            ...
            <x-text-input wire:model.live="minimumTransaction" id="minimumTransaction" class="block mt-1 w-full" type="number"
                name="minimumTransaction" placeholder="Minimum transaction" required />
        ...
        <div class="mb-4">
            ...
            <x-select-input :options="$courses" value="id" label="title" name="selectedCourse" wire:model.live="selectedCourse"
                wire:change="setSelectedCourse"></x-select-input>
            <x-input-error :messages="$errors->get('minimumOrder')" class="mt-2" />
            <div class="flex flex-wrap gap-2 @if(count($ruleCourses) > 0) mt-2 @endif">
                ...
                <span class="text-xs border rounded border-slate-400 p-1 flex items-center gap-2">{{ $selected->name }}
                    <i class="cursor-pointer bx bx-trash-alt text-red-500" wire:click="deleteSelectedCourse('{{ $selected->id }}')"></i>
                </span>
                @endforeach
            </div>
        </div>
        <div class="mb-4">
            ...
            <x-select-input :options="$members" value="id" label="name" name="selectedMember" wire:model.live="selectedMember"
                wire:change="setSelectedMember"></x-select-input>
            <x-input-error :messages="$errors->get('minimumOrder')" class="mt-2" />
        ...
        <div class="mb-4">
            <button wire:loading.attr="disabled" wire:target="submit"
                class="bg-emerald-500 px-6 py-3 text-sm text-white rounded hover:bg-emerald-600" type="submit"><span
                    wire:loading.remove wire:target="submit">Tambahkan voucher</span><span wire:loading
