    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-slate-50 rounded p-8 border border-slate-200">
            ...
            <div class="flex flex-col gap-4 mt-4">
                <div class="flex cursor-pointer items-center gap-4" wire:click="selectCategory('all')">
                    <div class="h-4 w-4 @if($selectedCategory == 'all') bg-sky-700 @else bg-slate-300 @endif rounded">
                    </div>
                    <p class="prose hover:text-sky-800">Semua kategori</p>
                </div>
                @forelse($categories as $category)
                <div class="flex cursor-pointer items-center gap-4" wire:key="{{ $category->id }}"
                    wire:click="selectCategory('{{ $category->id }}')">
                    <div
                        class="h-4 w-4 @if($selectedCategory == $category->id) bg-sky-700 @else bg-slate-300 @endif rounded">
                    </div>
            ...
            <div class="flex flex-col gap-4 mt-4">
                <div class="flex cursor-pointer items-center gap-4" wire:click="selectSort('terbaru')">
                    <div class="h-4 w-4 @if($selectedSort == 'terbaru') bg-sky-700 @else bg-slate-300 @endif rounded">
                    </div>
                    <p class="prose hover:text-sky-800">Terbaru</p>
                </div>
                <div class="flex cursor-pointer items-center gap-4" wire:click="selectSort('promo')">
                    <div class="h-4 w-4 @if($selectedSort == 'promo') bg-sky-700 @else bg-slate-300 @endif rounded">
                    </div>
                ...
                <div class="flex cursor-pointer items-center gap-4" wire:click="selectSort('terpopuler')">
                    <div
                        class="h-4 w-4 @if($selectedSort == 'terpopuler') bg-sky-700 @else bg-slate-300 @endif rounded">
                    </div>
        ...
        <div class="md:col-span-3">
            <div class="grid grid-cols-1 items-center gap-4">
                <x-text-input wire:model.live="query" id="query" class="block w-full h-16 text-lg px-4" type="query"
                    name="query" required placeholder="Cari berdasarkan judul atau nama Author" />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 mt-4 gap-4">
                ...
                <div class="h-full col-span-3">
                    <p class="text-slate-500">Ebook tidak ditemukan, silakan gunakan kata kunci atau filter yang lain
                    </p>
                </div>
                @endforelse
            </div>
