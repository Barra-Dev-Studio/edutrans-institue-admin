                ...
                <table class="table table-striped w-full text-left border border-gray-50 dark:border-zinc-600">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 dark:border-zinc-600 dark:bg-zinc-700/50 dark:text-zinc-100 bg-white prose"
                            wire:key="{{ $section->id }}">
                            <td class="p-3" width="50">{{ ($sections->currentpage()-1) * $sections->perpage() + $loop->index +
                                1 }}</td>
                            <td class="p-3">{{ $section->title }}</td>
                            <td class="p-3 flex">
                                <a href="{{ route('dashboard.section.show', $section->id) }}"
                                ...
                                <button wire:click="showModal('dashboard.section.destroy', '{{ $section->id }}')"
