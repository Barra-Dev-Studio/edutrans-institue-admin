                ...
                <table class="table table-striped w-full text-left border border-gray-50 dark:border-zinc-600">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 dark:border-zinc-600 dark:bg-zinc-700/50 dark:text-zinc-100 bg-white prose"
                            wire:key="{{ $quiz->id }}">
                            <td class="p-3" width="50">{{ ($quizzes->currentpage()-1) * $quizzes->perpage() + $loop->index +
                                1 }}</td>
                            <td class="p-3">{!! $quiz->question !!}</td>
                            <td class="p-3">
                                @foreach(json_decode($quiz->answers) as $answer)
                                    <p class="@if($answer->is_correct) text-emerald-500 @endif m-0">- {{ $answer->text }}</p>
                                @endforeach
                            </td>
                            <td class="p-3">{{ $quiz->score }}</td>
                            <td class="p-3">{{ $quiz->duration }}</td>
                            <td class="p-3">{{ $quiz->status }}</td>
                            <td class="p-3 flex">
                                <a href="{{ route('dashboard.quiz.edit', $quiz->id) }}"
                                    class="p-2 flex items-center bg-amber-400 hover:bg-amber-300 text-lg !no-underline rounded-bl rounded-tl">
                                    <i class="bx bx-edit"></i>
                                </a>
                                <button wire:click="showModal('dashboard.quiz.destroy', '{{ $quiz->id }}')"
